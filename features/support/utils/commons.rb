module Commons
  ...
  def swipe_page_down(swipe=250)
    if IOS
      Appium::TouchAction.new.swipe(start_x: 0 , start_y: 300, end_x: 0, end_y: -600, duration:1500).perform
    else
      Appium::TouchAction.new.swipe(start_x: 0 , start_y: 0.5, end_x: 0, end_y: swipe, duration:1500).perform
    end
  end

  def swipe_page_right(swipe=250)
    IOS ? $appium_driver.swipe(direction: 'left') : Appium::TouchAction.new.swipe(start_x: 0 , start_y: 0.5, end_x: swipe, end_y: 0, duration:1500).perform
  end
